...
// using BlockShape = cutlass::gemm::GemmShape<64, 128, 64>;
...
struct KernelTraits {
  ...
  static constexpr int kMCur = 1;
  static constexpr int kNCur = 2;
  static constexpr int kKCur = 1;

  static constexpr int kMmaPM = kMCur * kMmaEURepeatM * get<0>(mma_atom_shape{}); // 1 * 2 * 16 = 32
  static constexpr int kMmaPN = kNCur * kMmaEURepeatN * get<1>(mma_atom_shape{}); // 1 * 2 * 8 = 16
  static constexpr int kMmaPK = kKCur * kMmaEURepeatK * get<2>(mma_atom_shape{}); // 1 * 1 * 32 = 32
  using MMA = decltype(make_tiled_mma(
      mma_atom{},
      make_layout(
          Shape<Int<kMmaEURepeatM>, Int<kMmaEURepeatN>, Int<kMmaEURepeatK>>{}),
      Tile<Int<kMCur>, Int<kNCur>, Int<kKCur>>{}));
  ...
  using filter_s2r_copy_op = SM75_U32x4_LDSM_N;
  ...
  using SmemLayoutAtomC = decltype(composition(
      Swizzle<2, 3, 2>{},
      // Swizzle<2, 2, 3>{},
      make_layout(make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}), // 32 * 16
  ...
  static const int AlignOutput = 8;
  static const int e_s2g_copy_thr_contiguous = Int<kMmaPN>{} / AlignOutput;
  ...
  using e_s2g_copy_tile = decltype(make_tiled_copy(
      ...
      Layout<Shape<_1, Int<AlignOutput>>>{}));

  using e_c_s2g_copy_op = UniversalCopy<cutlass::AlignedArray<ElementC, AlignOutput>>;
...
__global__ void Conv(cutlass::conv::Conv2dProblemSize p, const typename KernelTraits::Element *a, const typename KernelTraits::Element *b,
                                typename KernelTraits::ElementC *c, ActivateParams ap, FilterParams fp, const typename KernelTraits::ElementCompute * device_scale,  const typename KernelTraits::ElementCompute * device_bias) {
#if 1
  ...
  auto output = make_tensor(make_gmem_ptr(c), make_shape(p.N*p.P*p.Q, p.K), make_stride(p.K, _1{}));
...
CUTLASS_PRAGMA_UNROLL
  for (int istages = 0; istages < kStage -1; ++istages) {
      ActivationG2S(istages);
      FilterG2S(istages);
  ...
  int ik = 0;
  // smem -> reg
      cute::copy(activation_s2r_tiled_copy, activation_s2r_tASA(_, _, ik, ismem_read),
                 thr_mma_tArA_view(_, _, ik));
      cute::copy(filter_s2r_tiled_copy, filter_s2r_tFSF(_, _, ik, ismem_read),
                 thr_mma_tFrF_view(_, _, ik));
    if (thread0()) {
      // PRINT(layout(activation_s2r_tASA));
      // PRINT(layout(thr_mma_tArA_view));
      // PRINT(layout(filter_s2r_tFSF));
      // PRINT(layout(thr_mma_tFrF_view));
    }


    clear(thr_mma_tCrC);
    int ntile = p.R*p.S * cute::ceil_div(p.C, Int<KernelTraits::BlockShape::kK>{});

    ///< main loop
    for (int itile = 0; itile < ntile; ++itile) {
      int nk = size<2>(thr_mma_tArA);
      ...
      cute::copy(activation_s2r_tiled_copy, activation_s2r_tASA(_, _, ik_next, ismem_read),
                 thr_mma_tArA_view(_, _, ik_next));
      cute::copy(filter_s2r_tiled_copy, filter_s2r_tFSF(_, _, ik_next, ismem_read),
                 thr_mma_tFrF_view(_, _, ik_next));

      if (ik == 0) {
        if (itile_to_read < ntile) {

          ActivationG2S(ismem_write);
          FilterG2S(ismem_write);
      ...
      cute::gemm(tiled_mma, thr_mma_tCrC, thr_mma_tArA(_, _, ik), thr_mma_tFrF(_, _, ik),
                 thr_mma_tCrC);
      if (thread0()) {
  ...
  constexpr int step = size<3>(tCsC_r2s); // pipe
#pragma unroll
  for (int i = 0; i < size<1>(tCrC_r2sx); i += step) {
    // reg -> shm
#pragma unroll
    for (int j = 0; j < step; ++j) {
      // auto t = make_tensor_like<ElementAccumulator>(tCrC_r2sx(_, i + j));
      // cute::copy(tCrC_r2sx(_, i + j), t);
      cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, i + j), tCsC_r2s(_, 0, 0, j));
      // if (thread0() && i == 0) {
      //   PRINT(tCrC_r2sx(_, i + j));
      //   PRINT(tCsC_r2s(_, 0, 0, j));
      // }
      // cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, i + j), tCsC_r2s(_, 0, 0, j));
...
#pragma unroll
    ...
    for (int j = 0; j < step; ++j) {
      ...
      auto t = make_tensor_like<ElementC>(tCgC_s2gx(_, i + j));
...
#pragma unroll
          for (int idx = 0; idx < size(t); ++idx) {
            t(idx) = (ElementC)((ElementCompute)tCsC_s2g(make_coord(idx, 0), 0,
                                                         0, j) *
                                    tCgScale(make_coord(idx, 0), 0,
                                             (i + j) / size<1>(tCgC_s2g)) +
                                tCgBias(make_coord(idx, 0), 0,
                                        (i + j) / size<1>(tCgC_s2g)));
          }

          cute::copy(s2g_c_tiled_copy_c, t, tCgC_s2gx(_, i + j));
...
int main() {
  ...
  printf("p:%d, q:%d\n", P, Q);
  using KT = KernelTraits<Element, Element, ElementC, ElementCompute, ElementAccumulator, ElementC, BlockShape, WarpShape, kStage, 16, 4>;
  //N, H, W, C, P, Q, K, R, S;
...
// return 0;
  ...
  Conv<KT><<<grid, block, smem_s>>>(p, (const Element *)A.second, (const Element *)B.second,
                                (ElementC *)C.second, ap, fp, (const ElementCompute *)hscale.second,  (const ElementCompute *)hbias.second);
