...
struct KernelTraits {
  static const int kSmemLayoutCBatch = 2;
  ...
  using SmemLayoutAtomC = decltype(composition(
      Swizzle<3, 2, 3>{}, make_layout(make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}), // 32 * 16
  ...
  using SmemLayoutCS2G = decltype(tile_to_shape(
      SmemLayoutAtomC{},
      make_shape(Int<kMmaPM>{}, Int<kMmaPN * kSmemLayoutCBatch>{}))); // 32 16 1
  ...
  using e_s2g_copy_tile = decltype(make_tiled_copy(
      ...
      e_s2g_copy_atom{}, make_layout(Shape<Int<kMmaPM>,Int<EpiJoinThreadNum / kMmaPM>>{}, Stride<_1, Int<kMmaPM>>{}),
  ...
  using e_c_s2g_copy_tile = decltype(make_tiled_copy(
      ...
      e_c_s2g_copy_atom{}, make_layout(Shape<Int<kMmaPM>,Int<EpiJoinThreadNum / kMmaPM>>{}, Stride<_1, Int<kMmaPM>>{}),
      Layout<Shape<_1, Int<AlignC>>>{}));
  ...
  using e_scale_s2g_copy_tile = decltype(make_tiled_copy(
      ...
      e_scale_s2g_copy_atom{}, make_layout(Shape<Int<kMmaPM>,Int<EpiJoinThreadNum / kMmaPM>>{}, Stride<_1, Int<kMmaPM>>{}),
      Layout<Shape<_1, Int<AlignC>>>{}));
...
asm volatile("mov.u32 %0, %%clock;" : "=r"(end) :: "memory");
  ...
  unsigned int start1, end1;
    ...
    auto e_smem_s2g = make_tensor(make_smem_ptr((ElementAccumulator*)(&e_s.epilogue_smem)), typename KernelTraits::SmemLayoutCS2G{});
    ...
    auto tCsC_s2g = s2g_thr_copy_c.partition_S(e_smem_s2g);
    ...
    if (thread0()){
      ...
      // PRINT(layout(tCrScale));
      // PRINT(layout(tCrBias));
    }
...
#pragma unroll
  for (int j = 0; j < size<2>(tCgC_s2g); ++j) {
        cute::copy(s2g_scale_tiled_copy_c, tCgScale(_, 0, j), tCrScale(_, 0, j));
        cute::copy(s2g_scale_tiled_copy_c, tCgBias(_, 0, j), tCrBias(_, 0, j));
#pragma unroll
    for (int i = 0; i < size<1>(tCgC_s2g); ++i) {
      // reg -> shm
      #pragma unroll
          for (int s = 0; s < step; ++s) {
            cute::copy(r2s_tiled_copy_c, tCrC_r2s(_, i, j * step + s), tCsC_r2s(_, 0,0, s));

          }
          __syncthreads();

      auto t = make_tensor_like<ElementC>(tCsC_s2g(_, 0, 0));
      //s2g
      if (need_pred) {
        // printf("need pred blockIdx.x:%d, blockIdx.y:%d\n", blockIdx.x,  blockIdx.y);
...
#pragma unroll
        for (int idx = 0; idx < size(t); ++idx) {
          t(idx) =
              (ElementC)((ElementCompute)tCsC_s2g(idx, 0, 0) *
                             tCrScale(make_coord(idx, 0), 0,j) +
                         tCrBias(make_coord(idx, 0), 0, j));
        }
        cutlass::arch::global_store<decltype(t), size(t) * sizeof(ElementC)>(t, tCgC_s2g(_, i, j).data().get(), true);
