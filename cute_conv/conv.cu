...
__global__ void Conv(cutlass::conv::Conv2dProblemSize p, const typename KernelTraits::Element *a, const typename KernelTraits::Element *b,
                                typename KernelTraits::ElementC *c, ActivateParams ap, FilterParams fp) {
#if 1
  ...
  auto smem_activate =
      make_tensor(make_smem_ptr((Element *)(&s.activate_smem)),
                  make_layout(Shape<Int<BlockShape::kM>, Int<BlockShape::kK>>{},
                              Stride<Int<BlockShape::kK>, _1>{}));
  auto smem_filter =
      make_tensor(make_smem_ptr((Element *)(&s.filter_smem)),
                  make_layout(Shape<Int<BlockShape::kN>, Int<BlockShape::kK>>{},
                              Stride<Int<BlockShape::kK>, _1>{}));
  ...
  typename KernelTraits::Implit_B_Thr_Coord_Layout implit_B_thr_coord_layout;
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      for (int i = 0; i < KernelTraits::ActivationkStrided; ++i) {
        auto activate_thr_tensor = make_tensor(make_gmem_ptr((Element*)(activate_pointer[i])), make_shape(Align));
        if (activate_thr_valid(i)) {
          // if (threadIdx.x == 127 && blockIdx.x == 0 && blockIdx.y == 0) {
          //   printf("i:%d\n", i);
          // }
          cute::copy(activate_thr_tensor, activate_g2s_thr_copy_D(_, i, 0));
        } else {
          // if (blockIdx.x == 0 && blockIdx.y == 0) {
          //   printf("threadIdx.x:%d\n", threadIdx.x);
          // }
          clear(activate_g2s_thr_copy_D(_, i, 0));
          // #pragma unroll
          // for (int j = 0; j < Align; ++j) {
          //   activate_g2s_thr_copy_D(j, i, 0) = 0;
          // }
    ...
    ///< filter g2s
      if (thread0()) {
            // printf("filter_point begin:%p\n", filter_point);
          }
      CUTLASS_PRAGMA_UNROLL
      for (int i = 0; i < KernelTraits::FilterkStrided; ++i) {
        auto filter_thr_tensor = make_tensor(make_gmem_ptr((Element*)(filter_point)), make_shape(Align));
        if (filter_thr_valid(i)) {
          if (blockIdx.x == 0 && blockIdx.y == 0) {
            // printf("threadIdx.x:%d, i:%d\n", threadIdx.x, i);
          }
            // PRINT_TENSOR(filter_thr_tensor);
          if (thread0()) {
            // printf(" invalid idx: %d\n", i);
          }
          cute::copy(filter_thr_tensor, filter_g2s_thr_copy_D(_, i, 0));
        } else {
          // printf("threadIdx.x:%d\n", threadIdx.x);
          clear(filter_g2s_thr_copy_D(_, i, 0));
        }
        if (i <=  KernelTraits::FilterkStrided - 2) {
          filter_point += fp.inc_next_k;
          if (thread0()) {
            // printf("filter_point incre:%p\n", filter_point);
          }
        }
      }
      cp_async_fence();
...
int main() {
  ...
  using KT = KernelTraits<Element, Element, ElementC, Element, ElementAccumulator, ElementC, BlockShape, WarpShape, Stages, 16, 1>;
  //N, H, W, C, P, Q, K, R, S;
