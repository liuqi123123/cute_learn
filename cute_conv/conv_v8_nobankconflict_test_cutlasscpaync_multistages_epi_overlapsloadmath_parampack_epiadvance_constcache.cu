...
struct KernelTraits {
  ...
  using e_s2g_copy_tile = decltype(make_tiled_copy(
      ...
      e_s2g_copy_atom{}, make_layout(Shape<Int<EpiJoinThreadNum / e_s2g_copy_thr_contiguous>,
                                      ...
                                      Int<e_s2g_copy_thr_contiguous>>{}, Stride<_1, Int<EpiJoinThreadNum / e_s2g_copy_thr_contiguous>>{}),
...
struct ParamsPack {
  ...
  ActivateParams const ap;
   ...
   ElementCompute device_scale[256];
   ElementCompute device_bias[256];
  // const ElementCompute *device_scale;
  // const ElementCompute *device_bias;
  uint32_t *device_time_info;

  ParamsPack( cutlass::conv::Conv2dProblemSize p_,
  const Element *a_,
  const Element *b_,
  ElementC *c_,
  ActivateParams ap_,
   FilterParams fp_,
  const ElementCompute *device_scale_,
  const ElementCompute *device_bias_,
  int k,
  uint32_t *device_time_info_) : p(p_), a(a_), b(b_), c(c_), ap(ap_), fp(fp_), device_time_info(device_time_info_) {
    for (int i = 0; i < k; ++i) {
      device_scale[i] = device_scale_[i];
      device_bias[i] = device_bias_[i];
    }
  }
};
...
#pragma unroll
    for (int j = 0; j < step; ++j) {
      auto t = make_tensor_like<ElementC>(tCrC_s2g(_, 0, 0, j));
...
#pragma unroll
          for (int idx = 0; idx < size(t); ++idx) {
            t(idx) = (ElementC)((ElementCompute)tCrC_s2g(make_coord(idx, 0), 0,
                                                         0, j) *
        ...
        for (int idx = 0; idx < size(t); ++idx) {
          t(idx) =
              (ElementC)((ElementCompute)tCrC_s2g(make_coord(idx, 0), 0, 0, j) *
                             tCgScale(make_coord(idx, 0), 0,
                                      (i + j) / size<1>(tCgC_s2g)) +
                         tCgBias(make_coord(idx, 0), 0,
                                 (i + j) / size<1>(tCgC_s2g)));
...
// return 0;
  ...
  ParamsPack params_pack{p,
                         ...
                         ap,
                         fp,
                         (const ElementCompute *)hscale.first,
                         (const ElementCompute *)hbias.first,
                         K,
