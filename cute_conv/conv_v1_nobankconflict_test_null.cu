...
struct KernelTraits {
  ...
  using SmemLayoutAtomA = decltype(composition(
    ...
    // Swizzle<0, 0, 0>{}, make_layout(Shape<Int<BlockShape::kM>, Int<BlockShape::kK>>{},
  ...
  using SmemLayoutAtomF = decltype(composition(
    ...
    // Swizzle<0, 0, 0>{}, make_layout(Shape<Int<BlockShape::kN>, Int<BlockShape::kK>>{},
                              Stride<Int<BlockShape::kK>, _1>{})));
  ...
  typename KernelTraits::Implit_B_Thr_Coord_Layout implit_B_thr_coord_layout;
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      for (int i = 0; i < KernelTraits::ActivationkStrided; ++i) {
        const char* target_activate_thr_tensor_addr = activate_thr_valid(i) ? activate_pointer[i]: reinterpret_cast<const char*>(null);
        auto activate_thr_tensor = make_tensor(make_gmem_ptr((Element*)(target_activate_thr_tensor_addr)), make_shape(Int<Align>{}));
    ...
    ///< filter g2s
      ...
      for (int i = 0; i < KernelTraits::FilterkStrided; ++i) {
        const char* target_filter_thr_tensor_addr = filter_thr_valid(i) ? filter_point: reinterpret_cast<const char*>(null);
        auto filter_thr_tensor = make_tensor(make_gmem_ptr((Element*)(target_filter_thr_tensor_addr)), make_shape(Int<Align>{}));
