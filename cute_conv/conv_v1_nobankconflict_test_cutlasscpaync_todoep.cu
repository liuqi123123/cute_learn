...
__global__ void Conv(cutlass::conv::Conv2dProblemSize p, const typename KernelTraits::Element *a, const typename KernelTraits::Element *b,
  ...
  typename KernelTraits::e_c_s2g_copy_tile epi_c_s2g_copy_tile;
  auto epi_c_s2g_thr_copy = epi_c_s2g_copy_tile.get_slice(threadIdx.x);
  auto epi_c_s2g_tEGE = epi_c_s2g_thr_copy.partition_D(block_output);
  ...
  if (thread0()) {
    // PRINT(convert_epi_s2g_tESE);
    // PRINT(epi_c_s2g_tEGE);
  }
  ...
  CUTLASS_PRAGMA_UNROLL
    for (int i = 0; i < size<1>(epi_s2g_tESE); ++i) {
      for (int j = 0; j < size<2>(epi_s2g_tESE); ++j) {
        if (tOrM(i) && tOrN(j)) {
        cute::copy(convert_epi_s2g_tESE(_, i, j), epi_c_s2g_tEGE(_, i, j));
