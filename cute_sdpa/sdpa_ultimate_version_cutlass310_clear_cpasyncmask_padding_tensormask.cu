...
#pragma unroll
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    if constexpr (TensorMask) {
      ...
      auto tensor_mask =
          make_tensor(make_gmem_ptr(mask), make_shape(params.S, params.Align8_T),
                      make_stride(params.Align8_T, Int<1>{}));
      ...
      //     });
      auto h2_thr_tCrC = recast<half2>(thr_tCrC);
      auto h2_s2r_tMrM = recast<half2>(s2r_tMrM);
      Operator_3D_Regs<size<0>(h2_thr_tCrC), size<1>(h2_thr_tCrC), size<2>(h2_thr_tCrC)>(
          [&](int i, int j, int m) {
            h2_thr_tCrC(make_coord(0, i), j, m) +=
                h2_s2r_tMrM(make_coord(0, i), j, m);
          });
      // if(thread0()) {
      //   PRINT(layout(thr_tCrC));
      //   PRINT(layout(h2_thr_tCrC));
      //   PRINT(layout(h2_s2r_tMrM));
      // }
...
int main() {
  ...
  // int N = 1, S = 1020*5, T = 1016*5, F = 128, H = 8;
  ...
  // int align8_T = (T + 7) / 8 * 8;
  auto Mask = helper.GetCpuGpuBuffer(S * problem.Align8_T, InitialType::PaddingDim0, T, problem. Align8_T);
...
for (int h = 0; h < H; ++h) {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    for (int i = 0; i < m; ++i) {
      for (int j = 0; j < n; ++j) {
        each_head_cpu_C_gt[i * n + j] +=  mask[i * problem.Align8_T + j];
      }
    }
  }
  for (int i = 0; i < m; ++i) {
