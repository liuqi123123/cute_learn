...
struct KernelTraits {
  ...
  //why V need NoSwizzle, but Q and K don't need?
  //when V don't do NoSwizzle, can't compile, but when Q K don't do NoSwizzle, can compile and get right result
  using SmemLayoutQNoSwizzle = decltype(get_nonswizzle_portion(SmemLayoutQ{}));
  using SmemLayoutKNoSwizzle = decltype(get_nonswizzle_portion(SmemLayoutK{}));
  using SmemLayoutVtransposedNoSwizzle = decltype(get_nonswizzle_portion(SmemLayoutV{}));
  using SmemLayoutEpiNoSwizzle = decltype(get_nonswizzle_portion(SmemLayoutEpilogue{}));

  using T_E = cutlass::AlignedArray<Element, Align>;
  using e_g2r_copy_op = UniversalCopy<T_E>;
  using e_g2r_traits = Copy_Traits<e_g2r_copy_op>;
  using e_g2r_copy_atom = Copy_Atom<e_g2r_traits, Element>;
  using e_g2r_copy_tile = decltype(make_tiled_copy(
      e_g2r_copy_atom{},
  ...
  // static constexpr int kMmaPN1 = 1 * kMmaEURepeatN * get<1>(mma_atom_shape{}) * Split_kF / 8;
...
// }
  ...
  Tensor smem_q_NoSwizzle = make_tensor(smem_q.data().get(), typename KernelTraits::SmemLayoutQNoSwizzle{});
  Tensor smem_k_NoSwizzle = make_tensor(smem_k.data().get(), typename KernelTraits::SmemLayoutKNoSwizzle{});
  Tensor smem_v_NoSwizzle = make_tensor(smem_v.data().get(), typename KernelTraits::SmemLayoutVtransposedNoSwizzle{});
...
// }
  ...
  // auto thr_tQrQ = thr_mma.partition_fragment_A(smem_q); // (MMA, MMA_M, MMA_K)
  auto thr_tQrQ = thr_mma.partition_fragment_A(smem_q_NoSwizzle); // (MMA, MMA_M, MMA_K)
  // auto tBrB = thr_mma.partition_fragment_B(gB(_, _, 0)); // (MMA, MMA_N,
  // MMA_K)
  // auto thr_tKrK = ( thr_mma.partition_fragment_B(smem_k(_, _, 0))); // (MMA, MMA_N, MMA_K)
  auto thr_tKrK = ( thr_mma.partition_fragment_B(smem_k_NoSwizzle(_, _, 0))); // (MMA, MMA_N, MMA_K)
...
#pragma unroll
  ...
  auto exp_tcrc = make_tensor<Element>(shape(thr_tCrC));
...
#pragma unroll
      for (int m = 0; m < 2; ++m) {
        ...
        *(exp_tcrc.data() + offset) = cutlass::fast_exp((*(thr_tCrC.data() + offset)) - (Element)tCrC_max[j]);
...
#pragma unroll
  ...
  auto mma1_tVrV = thr_mma1.partition_fragment_B(smem_v_NoSwizzle(_, _, 0));
...
#pragma unroll
    ...
    exp_oldm_sub_newm[0] = cutlass::fast_exp(tCrC_max[0] - new_m_max[0]);
    exp_oldm_sub_newm[1] = cutlass::fast_exp(tCrC_max[1] - new_m_max[1]);



    // auto new_exp_tcrc = make_tensor<ElementAccumulator>(shape(thr_tCrC));
    auto new_exp_tcrc = make_tensor<Element>(shape(thr_tCrC));
...
#pragma unroll
        ...
        const int offset = m + j * 2 + i * 4;
        *(new_exp_tcrc.data() + offset) = cutlass::fast_exp((*(thr_tCrC.data() + offset)) - static_cast<Element>(new_m_max[j]));
...
#pragma unroll
        ...
        *(thr_tCrC.data() + offset) = static_cast<Element>(cutlass::fast_exp((
            (*(thr_tCrC.data() + offset)) - static_cast<Element>(new_m_max[j]))));
        // if (thread0()) {
        //   printf("thr_tCrC((%d, %d), 0, %d): %f\n", m, j, i,
...
#pragma unroll
  ...
  __syncthreads();
  typename KernelTraits::e_g2r_copy_tile epi_smem_copy_tile;
...
int main() {
  ...
  int N = 1, S = 64, T = 1024, F = 128, H = 1;
  // int N = 1, S = 16, T = 32, F = 32, H = 1;
  MultiHeadAttentionProblemSize problem(N, S, T, kE, F, H);
  using KernelT = KernelTraits<cutlass::half_t, float, 8, 32, 128, 3>;
  ...
  auto l0 = make_layout(Shape<_32, _32>{}, Stride<_32, _1>{});
  auto shape = Shape<Underscore, _2>{};
  auto t1 = make_tensor((Element*)Q.first, l0);
  auto l1 = logical_divide(l0, shape);
  auto l2 = zipped_divide(t1, shape);
  // auto l3 = tiled_divide(l0, shape);
  PRINT(l0);
  PRINT(l1);
  PRINT(l2);
  // PRINT(l3);
