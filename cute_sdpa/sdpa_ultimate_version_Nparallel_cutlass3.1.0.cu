...
struct KernelTraits {
  ...
  using SmemLayoutVtransposedNoSwizzle = decltype(make_layout(Shape<Int<Split_kF>, Int<ThreadblockShape1::kK>>{}, Stride<_1, Int<Split_kF>>{}));
...
int main() {
  ...
  // int N = 4, S = 1111, T = 1111, F = 104, H = 8;
  int N = 5, S = 100, T = 325, F = 32, H = 4;
  // int N = 1, S = 16, T = 32, F = 32, H = 1;
  MultiHeadAttentionProblemSize problem(N, S, T, kE, F, H);
  using KernelT = KernelTraits<cutlass::half_t, float, 8, kE, 32, 1>;
