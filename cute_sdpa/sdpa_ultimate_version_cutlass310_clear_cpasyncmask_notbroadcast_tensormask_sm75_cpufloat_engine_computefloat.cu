...
#include "utils_engine.h"
#include "sdpa_problem_engine.cuh"
...
struct KernelTraits {
  ...
  using ThreadblockShape0 = cutlass::gemm::GemmShape<Split_kS, 64, kE>;
  using WarpShape0 = cutlass::gemm::GemmShape<16, ThreadblockShape0::kN, kE>;
  using ElementCompute = std::conditional_t<ArchTag::kMinComputeCapability == 80, Element, Element>;
  ...
  struct SharedStorageNoMasked {
    ...
    cutlass::AlignedArray<Element,
                          ThreadblockShape0::kN * ThreadblockShape0::kK>
        k_smem;
    cutlass::AlignedArray<Element,
                          ThreadblockShape1::kK * ThreadblockShape1::kN>
        v_smem;
  };
  ...
  using SharedStorage = typename std::conditional<Mask, SharedStorageMasked,
                                                  SharedStorageNoMasked>::type;

  struct EpilogueSharedStorage {
    cutlass::Array<Element, ThreadblockShape0::kM * Split_kF> e_smem;
  };

  static_assert(sizeof(SharedStorage) >= sizeof(EpilogueSharedStorage),
                "SharedStorage should be lager then epilogue shared smem");
  ...
  static_assert((32 * warp_num) % mask_continuous_thread == 0,
                "mask_continuous_thread must be divisible by (32 * warp_num)");

  static const int epilogue_continuous_thread = Split_kF / 8;
  // only ArchTag >= sm80 has cp_async
  using q_g2s_copy_op = typename std::conditional<
      ArchTag::kMinComputeCapability == 80,
      SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>,
      UniversalCopy<cutlass::AlignedArray<Element, 8>>>::type;

  using q_g2s_copy_traits = Copy_Traits<q_g2s_copy_op>;
  using q_g2s_copy_atom = Copy_Atom<q_g2s_copy_traits, Element>;

  using q_G2SCopy = decltype(make_tiled_copy(
      q_g2s_copy_atom{},
      make_layout(make_shape(Int<32 * warp_num / q_continuous_thread>{},
                             Int<q_continuous_thread>{}),
                  make_stride(Int<q_continuous_thread>{}, Int<1>{})),
      make_layout(make_shape(Int<1>{}, Int<Align>{}))));
  using k_G2SCopy = decltype(make_tiled_copy(
      q_g2s_copy_atom{},
      make_layout(make_shape(Int<32 * warp_num / q_continuous_thread>{},
                             Int<q_continuous_thread>{}),
                  make_stride(Int<q_continuous_thread>{}, Int<1>{})),
  ...
  using v_G2SCopy = decltype(make_tiled_copy(
      q_g2s_copy_atom{},
      make_layout(make_shape(Int<v_continuous_thread>{},
                             Int<32 * warp_num / v_continuous_thread>{}),
  ...
  using SmemLayoutQ = decltype(tile_to_shape(
      SmemLayoutAtomQ{}, Shape<Int<ThreadblockShape0::kM>, Int<kE>>{}));
  static const int K_swizzle_S = kE == 128 ? 4 : 3;

  using SmemLayoutAtomK = decltype(composition(
      // B M S
      Swizzle<3, 3, K_swizzle_S>{},
      make_layout(Shape<Int<ThreadblockShape0::kN>, Int<kE>>{},
                  Stride<Int<kE>, _1>{})));
  using SmemLayoutK = decltype(tile_to_shape(
      SmemLayoutAtomK{}, Shape<Int<ThreadblockShape0::kN>, Int<kE>>{}));
  static const int Mask_swizzle_S = ThreadblockShape0::kN == 128 ? 4 : 3;

  using SmemLayoutAtomMask = decltype(composition(
      // B M S
      Swizzle<3, 3, Mask_swizzle_S>{},
      make_layout(
          Shape<Int<ThreadblockShape0::kM>, Int<ThreadblockShape0::kN>>{},
  ...
  using SmemLayoutAtomV = decltype(composition(
      // B M S
      Swizzle<3, 3, V_swizzle_S>{},
      make_layout(Shape<Int<Split_kF>, Int<ThreadblockShape1::kK>>{},
                  Stride<_1, Int<Split_kF>>{})));
  using SmemLayoutV = decltype(tile_to_shape(
      SmemLayoutAtomV{}, Shape<Int<Split_kF>, Int<ThreadblockShape1::kK>>{}));
  // using SmemLayoutVtransposedNoSwizzle =
  // decltype(get_nonswizzle_portion(SmemLayoutV{}));
  using SmemLayoutVtransposedNoSwizzle =
      decltype(make_layout(Shape<Int<Split_kF>, Int<ThreadblockShape1::kK>>{},
                           Stride<_1, Int<Split_kF>>{}));
  ...
  using SmemLayoutEpilogue = decltype(composition(
      // B M S
      Swizzle<3, 3, Epi_swizzle_S>{},
      make_layout(Shape<Int<ThreadblockShape0::kM>, Int<Split_kF>>{},
  ...
  // using q_s2r_copy_op = SM75_U32x1_LDSM_N;
  using q_s2r_copy_op = SM75_U32x1_LDSM_N;
  ...
  // using k_s2r_copy_op = SM75_U32x1_LDSM_N;
  ...
  using v_s2r_copy_op = std::conditional_t<ArchTag::kMinComputeCapability == 80, SM75_U16x4_LDSM_T, SM75_U16x4_LDSM_T>;
  ...
  using mma_op = std::conditional_t<ArchTag::kMinComputeCapability == 80,
  ...
  static constexpr int kMmaEURepeatM = ThreadblockShape0::kM / WarpShape0::kM; // 2
  ...
  // static constexpr int kMmaPK = std::is_same_v<mma_op, SM80_16x8x16_F16F16F16F16_TN> ? 1 : 2;
  static constexpr int kMmaPK = std::is_same_v<mma_op, SM80_16x8x16_F16F16F16F16_TN> ? 1 : 2;
  ...
  static constexpr int kMmaPK1 = 2;
  using MMA1 = decltype(make_tiled_mma(
      mma_atom{},
      make_layout(
          Shape<Int<kMmaEURepeatM>, Int<kMmaEURepeatN>, Int<kMmaEURepeatK>>{}),
      make_layout(Tile<Int<kMmaPM1>, Int<kMmaPN1>, Int<kMmaPK1>>{})));
};

template <typename KernelTraits>
__global__ void SDPA(MultiHeadAttentionProblemSize problem,
                      typename KernelTraits::Element *q,
                      typename KernelTraits::Element *k,
                      typename KernelTraits::Element *v,
                      typename KernelTraits::Element *mask,
                    typename KernelTraits::Element *out, bool mask_broadcast) {
  ...
  auto global_q = make_tensor(
      make_gmem_ptr(q), make_shape(problem.N, problem.S, problem.H, Int<kE>{}),
      make_stride(problem.S * problem.H * Int<kE>{}, problem.H * Int<kE>{},
                  Int<kE>{}, _1{}));
  auto global_k = make_tensor(
      make_gmem_ptr(k), make_shape(problem.N, problem.T, problem.H, Int<kE>{}),
      make_stride(problem.T * problem.H * Int<kE>{}, problem.H * Int<kE>{},
                  Int<kE>{}, _1{}));

  auto global_v = make_tensor(
      make_gmem_ptr(v), make_shape(problem.N, problem.H, problem.F, problem.T),
      make_stride(problem.H * problem.F * problem.T, problem.F, _1{},
                  problem.H * problem.F));

  auto global_out =
      make_tensor(make_gmem_ptr(out),
                  make_shape(problem.N, problem.S, problem.H, problem.F),
                  make_stride(problem.S * problem.H * problem.F,
                              problem.H * problem.F, problem.F, _1{}));

  auto block_q = coalesce(local_tile(
      global_q,
      make_shape(Int<1>{}, Int<KernelTraits::ThreadblockShape0::kM>{}, Int<1>{},
                 Int<kE>{}),
      make_coord(blockIdx.z / problem.H, blockIdx.x, blockIdx.z % problem.H,
                 0)));

  auto block_k = coalesce(local_tile(
      global_k, make_shape(Int<1>{}, problem.T, Int<1>{}, Int<kE>{}),
      make_coord(blockIdx.z / problem.H, 0, blockIdx.z % problem.H, 0)));
  auto block_v = coalesce(local_tile(
      global_v, make_shape(Int<1>{}, Int<1>{}, Int<Split_kF>{}, problem.T),
      make_coord(blockIdx.z / problem.H, blockIdx.z % problem.H, blockIdx.y,
                 0)));
  ...
  auto smem_v_NoSwizzle =
      make_tensor(make_smem_ptr((Element *)&smem.v_smem),
  ...
  if ((blockIdx.x + 1) * KernelTraits::ThreadblockShape0::kM > problem.S) {
...
#pragma unroll
    for (int i = 0; i < size<1>(tQiQ); ++i) {
      tQpQ(i) = (int)get<0>(tQiQ(make_coord(0, 0), i, 0)) <
                problem.S - blockIdx.x * KernelTraits::ThreadblockShape0::kM;
    }

    Operator_2D_Regs<size<1>(tQiQ), size<2>(tQiQ)>([&](int i, int j) {
      if (tQpQ(i)) {
        copy(q_tiled_copy, tQgQ(_, i, j), tQsQ(_, i, j));
      }
    });
  } else {
  ...
  CP_ASYNC_FENCE<typename KernelTraits::ArchTag>();
...
#pragma unroll
  for (int i = 0; i < size<1>(tKiK); ++i) {
    tKpK(i) = (int)get<0>(tKiK(make_coord(0, 0), i, 0)) <
              problem.T - KernelTraits::ThreadblockShape0::kN * 0;
  }

  Operator_2D_Regs<size<1>(tKiK), size<2>(tKiK)>([&](int i, int j) {
    if (tKpK(i)) {
      copy(k_tiled_copy, tKgK(_, i, j), tKsK(_, i, j));
    }
  });
  // __syncthreads();
  ...
  auto s2r_tiled_copy_q =
      make_tiled_copy_A(typename KernelTraits::S2RCopyAtomQ{}, tiled_mma);
  auto s2r_thr_copy_q = s2r_tiled_copy_q.get_slice(threadIdx.x);
  auto thr_QsQ =
      s2r_thr_copy_q.partition_S(smem_q);  // ? (CPY, CPY_M, CPY_K, kStage)

  auto s2r_tiled_copy_k =
      make_tiled_copy_B(typename KernelTraits::S2RCopyAtomK{}, tiled_mma);
  auto s2r_thr_copy_k = s2r_tiled_copy_k.get_slice(threadIdx.x);
  auto thr_KsK =
      s2r_thr_copy_k.partition_S(smem_k);  // ? (CPY, CPY_M, CPY_K, kStage)
  ...
  auto thr_tCrC = partition_fragment_C(
      ...
      Shape<Int<KernelTraits::ThreadblockShape0::kM>,
            Int<KernelTraits::ThreadblockShape0::kN>>{});  // (MMA, MMA_M,
                                                           // MMA_N)
  ...
  CP_ASYNC_WAIT<typename KernelTraits::ArchTag, 1>();
  ...
  auto s2r_tiled_copy_v =
      make_tiled_copy_B(typename KernelTraits::S2RCopyAtomV{}, tiled_mma1);
  auto s2r_thr_copy_v = s2r_tiled_copy_v.get_slice(threadIdx.x);
  auto mma1_tVsV =
      s2r_thr_copy_v.partition_S(smem_v);  // ? (CPY, CPY_M, CPY_K, kStage)



  using O_type = typename decltype(mma1_tOrO)::value_type;
  ...
  int ntile = (problem.T + KernelTraits::ThreadblockShape0::kN - 1) /
              KernelTraits::ThreadblockShape0::kN;

  ///< main loop
  for (int itile = 0; itile < ntile; ++itile) {
    CP_ASYNC_WAIT<typename KernelTraits::ArchTag, 0>();
    __syncthreads();
    ...
    if ((blockIdx.y + 1) * Split_kF > problem.F ||
        (itile + 1) * KernelTraits::ThreadblockShape1::kK >= problem.T) {
      ///< V g2s before  gemm Q@K
      Operator_2D_Regs<size<1>(tViV), size<2>(tViV)>([&](int i, int j) {
        tVpV(i, j) =
            (int)get<1>(tViV(make_coord(0, 0), i, j)) <
                problem.T - itile * KernelTraits::ThreadblockShape1::kK &&
            (int)get<0>(tViV(make_coord(0, 0), i, j)) <
                problem.F - blockIdx.y * Split_kF;
      });
  ...
  // __syncthreads();
    ...
    axpby(1, thr_tCrC, _0{}, converted_thr_tCrC);

    // Tensor converted_thr_tCrC = convert_type<Element>(thr_tCrC);


    ///< scale
    Operator_3D_Regs<size<0>(converted_thr_tCrC), size<1>(converted_thr_tCrC), size<2>(converted_thr_tCrC)>(
        [&](int i, int j, int m) {
          converted_thr_tCrC(make_coord(i % 2, i / 2), j, m) *= (ElementCompute)problem.Scale;
        });
    ///< Mask mode
    if constexpr (TensorMask) {
      ...
      // if (blockIdx.x == gridDim.x - 1 ||
      if ((blockIdx.x + 1) * KernelTraits::ThreadblockShape0::kM > problem.S ||
          (itile + 1) * KernelTraits::ThreadblockShape0::kN >= problem.T) {
        auto tMpM =
        ...
        Operator_2D_Regs<size<1>(g2s_tMgM), size<2>(g2s_tMgM)>(
            [&](int i, int j) {
              tMpM(i, j) =
                  (int)get<0>(tMiM(make_coord(0, 0), i, j)) <
                      problem.S -
                          blockIdx.x * KernelTraits::ThreadblockShape0::kM &&
                  (int)get<1>(tMiM(make_coord(0, 0), i, j)) <
                      problem.T - itile * KernelTraits::ThreadblockShape0::kN;
            });
      ...
      auto s2r_tMrM = make_fragment_like<Element>(converted_thr_tCrC);
      ...
      Operator_3D_Regs<size<0>(converted_thr_tCrC), size<1>(converted_thr_tCrC), size<2>(converted_thr_tCrC)>(
          [&](int i, int j, int m) {
            converted_thr_tCrC(make_coord(i % 2, i / 2), j, m) +=
    ...
    int next_itile = itile + 1;
    tKgK = k_g2s_thr_copy.partition_S(tiled_block_k(_, _, next_itile % ntile));
    clear(tKsK);
    // if only has one tile, don't need copy k
    if (next_itile <= ntile - 1) {
      ///< next_k g2s copy after gemm Q@K, only last tile need mask
      if (itile + 1 == ntile - 1) {
#pragma unroll
        for (int i = 0; i < size<1>(tKiK); ++i) {
          ///< don't forget cast to int when compare, because type(get....) is
          ///< not int
          tKpK(i) =
              (int)get<0>(tKiK(make_coord(0, 0), i, 0)) <
              problem.T - KernelTraits::ThreadblockShape0::kN * (itile + 1);
        }
        ...
        ///< next_k g2s copy after gemm Q@K, tile other than last don't need
        ///< mask
  ...
  __syncthreads();
    ...
    if (itile == ntile - 1) {
      auto C_identity = make_identity_tensor(
          Shape<Int<KernelTraits::ThreadblockShape0::kM>,
                Int<KernelTraits::ThreadblockShape0::kN>>{});

      auto tCiC = thr_mma.partition_C(C_identity);
      auto tCpC =
          make_tensor<bool>(make_shape(_2{}, size<1>(tCiC), size<2>(tCiC)));
      if (thread0()) {
        // PRINT(layout(tCiC));
        // print_tensor((tCiC));
        // PRINT(tCpC);
      ...
      Operator_3D_Regs<size<0>(converted_thr_tCrC), size<1>(converted_thr_tCrC), size<2>(converted_thr_tCrC)>(
          [&](int i, int j, int m) {
            if (tCpC(i % 2, j, m)) {
              converted_thr_tCrC(make_coord(i % 2, i / 2), j, m) =
                  -platform::numeric_limits<ElementCompute>::infinity();  // half -inf
            }
          });
    }
    ...
    // to do @liuqi rm argument size<2>(converted_thr_tCrC)!
    Operator_Softmax<Element, ElementAccumulator,ElementCompute, size<2>(converted_thr_tCrC)>(converted_thr_tCrC, exp_oldm_sub_newm,
                                                  d, m_max, itile);

    ///< scale for O
    Operator_3D_Regs<size<2>(mma1_tOrO), 2, 2>([&](int i, int j, int m) {
      mma1_tOrO(make_coord(m, j), 0, i) =
          mma1_tOrO(make_coord(m, j), 0, i) *
          static_cast<O_type>(exp_oldm_sub_newm[j]);
    });

    CP_ASYNC_WAIT<typename KernelTraits::ArchTag, 1>();

    // __syncthreads();
    auto mma1_tVrV_view = s2r_thr_copy_v.retile_D(mma1_tVrV);
    if (thread0()) {
      PRINT(layout(mma1_tVrV));
      PRINT(layout(mma1_tVrV_view));
    }
    cute::copy(s2r_tiled_copy_v, mma1_tVsV, mma1_tVrV_view);
    ///< gemm  QK @ V
    Tensor tOrrC = make_tensor(
        converted_thr_tCrC.data(),
        // convert_layout_acc_Aregs<KernelTraits::mma_traits>(converted_thr_tCrC.layout()));
        Convert_layout_acc_Aregs< decltype(converted_thr_tCrC.layout()),
                                 get<2>(typename KernelTraits::mma_traits::Shape_MNK{})>::
            GetLayout(converted_thr_tCrC.layout()));

    auto trans_tOrrC = make_fragment_like<Element>(tOrrC);
    axpby(1, tOrrC, _0{}, trans_tOrrC);
    cute::gemm(tiled_mma1, mma1_tOrO, trans_tOrrC, mma1_tVrV, mma1_tOrO);
  }

  ///< last scale for O in softmax
  Operator_3D_Regs<size<2>(mma1_tOrO), 2, 2>([&](int i, int j, int m) {
    mma1_tOrO(make_coord(m, j), 0, i) =
        mma1_tOrO(make_coord(m, j), 0, i) / static_cast<O_type>(d[j]);
  });

  ///< epilogue
  using EpilogueSharedStorage = typename KernelTraits::EpilogueSharedStorage;
  EpilogueSharedStorage &epi_smem_storage =
      *(reinterpret_cast<EpilogueSharedStorage *>(s));

  Tensor e_smem =
      make_tensor(make_smem_ptr((Element *)&epi_smem_storage.e_smem),
                  typename KernelTraits::SmemLayoutEpilogue{});

  auto mma1_tOsO = thr_mma1.partition_C(e_smem);
  // Tensor convert_mma1_tOrO = convert_type<Element>(mma1_tOrO);
     Tensor convert_mma1_tOrO = make_fragment_like<Element>(mma1_tOrO);

    axpby(1, mma1_tOrO, _0{}, convert_mma1_tOrO);
  ///< epilogue reg -> smem
  cute::copy(convert_mma1_tOrO, mma1_tOsO);
  __syncthreads();
...
#pragma unroll
  for (int i = 0; i < size<0>(tEpE_S); ++i) {
    tEpE_S(i) = (int)get<0>(tEiE(make_coord(0, 0), i, 0)) +
                    blockIdx.x * KernelTraits::ThreadblockShape1::kM <
                problem.S;
  }

#pragma unroll
  for (int i = 0; i < size<0>(tEpE_F); ++i) {
    tEpE_F(i) = (int)get<1>(tEiE(make_coord(0, 0), 0, i)) +
                    blockIdx.y * KernelTraits::ThreadblockShape1::kN <
                problem.F;
  }
...
using KernelT = KernelTraits<UseMask, 32, 32, 1, 32, cutlass::arch::Sm75>;
  ...
  auto Mask = helper.GetCpuGpuBuffer(N * H* problem.PSize());
  ...
  SDPA<KernelT><<<grid, block, smem_s>>>(
      problem, (Element *)(Q.second), (Element *)(K.second),
      (Element *)(V.second),(Element*)(Mask.second), (Element *)(Out.second), false);
  ...
  ElementCompute *C_gt = (ElementCompute *)compute_helper.GetCpuBuffer(N * H *S * T);
  ElementCompute *m_gt = (ElementCompute *)compute_helper.GetCpuBuffer(N * H *S);
...
for (int h = 0; h < H; ++h) {
  ...
  Element* each_head_cpu_mask = each_N_cpu_mask + h * S * T;
  for (int i = 0 ; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      ...
      // printf("sum:%f\n", (float)sum);
      each_head_cpu_C_gt[i * n + j] = (ElementCompute)sum;
    }
  }
  //scale
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      each_head_cpu_C_gt[i * n + j] *= (ElementCompute)problem.Scale;
    }
  }
    if constexpr (UseMask) {
    for (int i = 0; i < m; ++i) {
      for (int j = 0; j < n; ++j) {
        each_head_cpu_C_gt[i * n + j] +=  (ElementCompute)each_head_cpu_mask[i * n + j];
      }
    }
  }
  for (int i = 0; i < m; ++i) {
  ...
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      ElementCompute temp = (ElementCompute)exp(each_head_cpu_C_gt[i * n + j] - each_head_cpu_m_gt[i]);
  ...
  for (int i = 0; i < m; ++i) {
    ...
    for (int j = 0; j < n; ++j) {
      each_head_cpu_C_gt[i * n + j] = each_head_cpu_C_gt[i * n + j] / (ElementCompute)sum;
    }
  }
  ...
  // helper.Regression(res_gt, (Element*)Out.first, true, true);
#endif
