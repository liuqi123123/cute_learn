...
struct KernelTraits {
  ...
  using ElementCompute =
      std::conditional_t<ArchTag::kMinComputeCapability == 80, Element,
                         Element>;
  ...
  static constexpr int kMmaPK =
      std::is_same_v<mma_op, SM80_16x8x16_F16F16F16F16_TN> ? 1 : 2;
...
#pragma unroll
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    ///< scale
    Operator_3D_Regs<size<0>(converted_thr_tCrC), size<1>(converted_thr_tCrC),
                     size<2>(converted_thr_tCrC)>([&](int i, int j, int m) {
      converted_thr_tCrC(make_coord(i % 2, i / 2), j, m) *=
          (ElementCompute)problem.Scale;
    });
    ///< Mask mode
...
#pragma unroll
    ...
    if (itile == ntile - 1) {
      ...
      Operator_3D_Regs<size<0>(converted_thr_tCrC), size<1>(converted_thr_tCrC),
        ...
        if (tCpC(i % 2, j, m)) {
          converted_thr_tCrC(make_coord(i % 2, i / 2), j, m) =
              -platform::numeric_limits<ElementCompute>::infinity();  // half
                                                                      // -inf
        }
      });
    }
    ...
    Operator_Softmax<Element, ElementAccumulator, ElementCompute,
                     size<2>(converted_thr_tCrC)>(
        converted_thr_tCrC, exp_oldm_sub_newm, d, m_max, itile);
    ...
    Tensor tOrrC = make_tensor(
        ...
        Convert_layout_acc_Aregs<
            decltype(converted_thr_tCrC.layout()),
            get<2>(typename KernelTraits::mma_traits::Shape_MNK{})>::
            Invoke(converted_thr_tCrC.layout()));
