...
struct KernelTraits {
  ...
  using v_s2r_copy_op = std::conditional_t<ArchTag::kMinComputeCapability == 80, SM75_U16x4_LDSM_T, SM75_U16x2_LDSM_T>;
...
#pragma unroll
  ...
  // __syncthreads();
    ...
    Tensor converted_thr_tCrC = make_fragment_like<Element>(thr_tCrC);
    ...
    Operator_3D_Regs<size<0>(converted_thr_tCrC), size<1>(converted_thr_tCrC), size<2>(converted_thr_tCrC)>(
        [&](int i, int j, int m) {
          converted_thr_tCrC(make_coord(i % 2, i / 2), j, m) *= (Element)problem.Scale;
        });
    ///< Mask mode
    if constexpr (TensorMask) {
      ...
      auto s2r_tMrM = make_fragment_like(converted_thr_tCrC);
    ...
    if (next_itile <= ntile - 1) {
      ...
      if (problem.T - KernelTraits::ThreadblockShape0::kN * (next_itile + 1) < 0) {
#pragma unroll
        for (int i = 0; i < size<1>(tKiK); ++i) {
  ...
  __syncthreads();
    ...
    Operator_Softmax<Element, ElementAccumulator,  size<2>(converted_thr_tCrC)>(converted_thr_tCrC, exp_oldm_sub_newm,
                                                  d, m_max, itile);
...
using KernelT = KernelTraits<false, 32, 32, 1, 32, cutlass::arch::Sm75>;
