...
template <typename Element_, typename ElementAccumulator_,typename ElementCompute_, int Align_, int kE_,
...
// }
  ...
  ElementCompute tCrC_max[2];
  #pragma unroll
  for (int i = 0; i < 2; ++i) {
    tCrC_max[i] = std::numeric_limits<half>::lowest();;
  }
...
#pragma unroll
      for (int m = 0; m < 2; ++m) {
        ...
        tCrC_max[j] = std::max((tCrC_max[j]), (ElementCompute)(*(thr_tCrC.data() + offset)));
      }
    }
  }
...
#pragma unroll
  for (int i = 1; i <= 2; i <<= 1) {
    tCrC_max[0] = std::max((ElementCompute)(__shfl_xor_sync(0xffffffff, tCrC_max[0], i)), tCrC_max[0]);
    tCrC_max[1] = std::max((ElementCompute)(__shfl_xor_sync(0xffffffff, tCrC_max[1], i)), tCrC_max[1]);
  }

  ElementCompute d[2]{ElementCompute(0.0f)};
  ElementAccumulator exp_tcrc_sum[2]{ElementAccumulator(0.0f)};

  auto exp_tcrc = make_tensor<ElementCompute>(shape(thr_tCrC));
...
#pragma unroll
      for (int m = 0; m < 2; ++m) {
        ...
        *(exp_tcrc.data() + offset) = exp((*(thr_tCrC.data() + offset)) - tCrC_max[j]);
...
#pragma unroll
  for (int i = 1; i <= 2; i <<= 1) {
    exp_tcrc_sum[0] += (ElementAccumulator)__shfl_xor_sync(0xffffffff, exp_tcrc_sum[0], i);
    exp_tcrc_sum[1] += (ElementAccumulator)__shfl_xor_sync(0xffffffff, exp_tcrc_sum[1], i);
  }

  d[0] = static_cast<ElementCompute>(exp_tcrc_sum[0]);
  d[1] = static_cast<ElementCompute>(exp_tcrc_sum[1]);
...
#pragma unroll
      for (int m = 0; m < 2; ++m) {
        ...
        *(thr_tCrC.data() + offset) = static_cast<Element>(*(exp_tcrc.data() + offset) / d[j]);
...
//     }
  ...
  // __syncthreads();
    ...
    ElementCompute new_tCrC_max[2];
    ElementCompute exp_oldm_sub_newm[2];
    ElementCompute new_m_max[2];
    ...
    ElementCompute new_d[2]{ElementCompute(0.0f)};
    ElementAccumulator new_exp_tcrc_sum[2]{ElementAccumulator(0.0f)};
    ElementCompute new_o_scale[2]{ElementCompute(0.0f)};
...
#pragma unroll
        for (int m = 0; m < 2; ++m) {
          const int offset = m + 2 * j + i * 4;
          new_tCrC_max[j] =
              std::max(new_tCrC_max[j], (ElementCompute)*(thr_tCrC.data() + offset));
        }
      }
    }
#pragma unroll
    for (int i = 1; i <= 2; i <<= 1) {
      ...
      new_tCrC_max[0] =
          std::max((ElementCompute)__shfl_xor_sync(0xffffffff, new_tCrC_max[0], i), new_tCrC_max[0]);
      new_tCrC_max[1] =
          std::max((ElementCompute)__shfl_xor_sync(0xffffffff, new_tCrC_max[1], i), new_tCrC_max[1]);
    }

    new_m_max[0] = std::max(tCrC_max[0], new_tCrC_max[0]);
    new_m_max[1] = std::max(tCrC_max[1], new_tCrC_max[1]);
    exp_oldm_sub_newm[0] = exp(tCrC_max[0] - new_m_max[0]);
    exp_oldm_sub_newm[1] = exp(tCrC_max[1] - new_m_max[1]);



    auto new_exp_tcrc = make_tensor<ElementCompute>(shape(thr_tCrC));
...
#pragma unroll
        ...
        const int offset = m + j * 2 + i * 4;
        *(new_exp_tcrc.data() + offset) = exp((ElementCompute)(*(thr_tCrC.data() + offset)) - new_m_max[j]);
...
#pragma unroll
    for (int i = 1; i <= 2; i <<= 1) {
      new_exp_tcrc_sum[0] +=
          (ElementAccumulator)__shfl_xor_sync(0xffffffff, new_exp_tcrc_sum[0], i);
      new_exp_tcrc_sum[1] +=
          (ElementAccumulator)__shfl_xor_sync(0xffffffff, new_exp_tcrc_sum[1], i);
    }
    new_o_scale[0] = d[0] * exp_oldm_sub_newm[0];
    new_o_scale[1] = d[1] * exp_oldm_sub_newm[1];

    new_d[0] = new_o_scale[0] + (ElementCompute)new_exp_tcrc_sum[0];
    new_d[1] = new_o_scale[1] + (ElementCompute)new_exp_tcrc_sum[1];
...
int main() {
  ...
  using KernelT = KernelTraits<cutlass::half_t, float, float, 8, 32, 32, 3>;
