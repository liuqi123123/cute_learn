...
struct KernelTraits {
  ...
  using q_g2s_copy_op = UniversalCopy<cutlass::AlignedArray<Element, 8>>;
  ...
  using v_s2r_copy_op = SM75_U16x2_LDSM_T;
  ...
  using mma_op = SM75_16x8x8_F32F16F16F32_TN;
...
if (thread0()) {
  // PRINT(layout(mma1_tArA));
  // PRINT(layout(mma1_tVrV));
}
// #if 1
  Tensor thr_tCrC1 = make_tensor(
      thr_tCrC.data(),
      convert_layout_acc_Aregs<KernelTraits::mma_traits>(thr_tCrC.layout()));
  // Tensor f16_tOrrC = make_fragment_like<Element>(thr_tCrC);
  // axpby(1, thr_tCrC, _0{}, f16_tOrrC);
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    Tensor f16_thr_tCrC = make_fragment_like<Element>(thr_tCrC);
    axpby(1, thr_tCrC, _0{}, f16_thr_tCrC);
    // if (thread0()) {
    //   PRINT(layout(thr_tCrC));
    //   PRINT(layout(f16_thr_tCrC));
    // }
    ///< scale
    Operator_3D_Regs<size<0>(f16_thr_tCrC), size<1>(f16_thr_tCrC), size<2>(f16_thr_tCrC)>(
      [&](int i, int j, int m){f16_thr_tCrC(make_coord(i%2, i/2), j, m) *= (Element)params.Scale;}
    );
    ///< Mask mode
    if constexpr (TensorMask) {
      ...
      auto tensor_mask =
          make_tensor(make_gmem_ptr(mask), make_shape(params.N * params.H, params.S, params.T),
                      make_stride(params.S * params.T, params.T, Int<1>{}));
      auto block_mask = coalesce(local_tile(tensor_mask, make_tile(_1{}, params.S, params.T), make_coord(blockIdx.z, _0{}, _0{})));
      ...
      decltype(f16_thr_tCrC) s2r_tMrM;
      if (thread0()) {
      ...
      Operator_3D_Regs<size<0>(f16_thr_tCrC), size<1>(f16_thr_tCrC), size<2>(f16_thr_tCrC)>(
          [&](int i, int j, int m) {
            f16_thr_tCrC(make_coord(i % 2, i / 2), j, m) +=
  ...
  if (ntile > 1) {
    ...
    if (params.T < KernelTraits::ThreadblockShape0::kN * (itile + 2)) {
      #pragma unroll
      for (int i = 0; i < size<1>(tKiK); ++i) {
    ...
    if (params.T < KernelTraits::ThreadblockShape0::kN * (itile + 1)) {
  ...
  Operator_3D_Regs<size<0>(f16_thr_tCrC), size<1>(f16_thr_tCrC), size<2>(f16_thr_tCrC)>(
    ...
    Operator_Softmax<Element, ElementAccumulator>(f16_thr_tCrC, exp_oldm_sub_newm, d, m_max, itile);
...
if (thread0()) {
  ...
  // PRINT(mma1_tOrO);
    ...
    cute::gemm(tiled_mma1, mma1_tOrO, f16_thr_tCrC, mma1_tVrV, mma1_tOrO);
    // cute::gemm(tiled_mma1, mma1_tOrO, tOrrC, retile_mma1_tVrV, mma1_tOrO);
...
Operator_3D_Regs<size<2>(mma1_tOrO), 2, 2>(
...
#if 1
  Tensor f16_mma1_tOrO = make_fragment_like<Element>(mma1_tOrO);
  axpby(1, mma1_tOrO, _0{}, f16_mma1_tOrO);
  ///< epilogue
  ...
  ///< epilogue reg -> smem
  cute::copy(f16_mma1_tOrO, mma1_tOsO);
  __syncthreads();
...
int main() {
  ...
  using KernelT = KernelTraits<cutlass::half_t, float, 8, kE, 32, UseMask, 1>;
