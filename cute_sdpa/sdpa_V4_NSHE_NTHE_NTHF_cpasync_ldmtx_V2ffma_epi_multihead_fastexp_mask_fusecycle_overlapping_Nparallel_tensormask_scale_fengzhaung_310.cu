...
struct KernelTraits {
  ...
  static constexpr int kMmaPN = 2;//这里的2是通过增加寄存器的方法，使得Atom在N方向扩展两倍
...
// #if 0
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    if constexpr (TensorMask) {
      ...
      // auto tiled_tensor_mask = local_tile(block_mask,
      // make_shape(Int<KernelTraits::ThreadblockShape0::kM>{},), make_coord(0,
      // _));
      if (thread0()) {
        // PRINT(tensor_mask);
        // PRINT(block_mask);
      ...
      // auto g2s_tMgM = mask_g2s_copy_thr.partition_S(tiled_tensor_mask(_,_,
      // 0));
      auto g2s_tMgM =
      ...
      Operator_2D_Regs<size<1>(g2s_tMgM), size<2>(g2s_tMgM)>([&](int i, int j) {
        tMpM(i, j) =
            get<0>(tMiM(make_coord(0, 0), i, j)) <
                params.S - blockIdx.x * KernelTraits::ThreadblockShape0::kM &&
            get<1>(tMiM(make_coord(0, 0), i, j)) <
      ...
      // Mask mode, for tiled_mma gemm and g2r mask copy
      __syncthreads();
    ...
    clear(tKsK);
    ///< next_k g2s copy after gemm Q@K
    copy(k_tiled_copy, tKgK, tKsK);
  ...
  auto C_identity =
      make_identity_tensor(Shape<Int<KernelTraits::ThreadblockShape0::kM>,
                                 Int<KernelTraits::ThreadblockShape0::kN>>{});
  // auto s2r_c_identity_copy = make_tiled_copy_C(typename
  // KernelTraits::S2RCopyAtomIdentityC{}, tiled_mma); auto
  // s2r_c_identity_thr_copy = s2r_c_identity_copy.get_slice(threadIdx.x); auto
  // tCiC = s2r_c_identity_thr_copy.partition_S(C_identity);
...
int main() {
  ...
  int N = 2, S = 128, T = 120, F = 128, H = 2;
  // int N = 1, S = 1024*5, T = 1024*5, F = 128, H = 8;
  // int N = 2, S = 333, T = 1112, F = 104, H = 2;
