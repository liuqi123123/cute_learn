...
int main() {
  ...
  int m0 = 1, n0 = 128, k0 = 3584;
  int m1 = 1, n1 = 3  , k1 = 3584;
  int m2 = 1, n2 = 128, k2 = 3584;
  int m3 = 1, n3 = 3 ,  k3 = 3584;
  int m4 = 1, n4 = 128, k4 = 3584;
  const int ab_align = 8;
  const int c_align = 1;
  ...
  auto gemm2_A = helper.GetCpuGpuBuffer(m2 * k2);
  auto gemm2_B = helper.GetCpuGpuBuffer(n2 * k2);
  auto gemm2_C = helper.GetCpuGpuBuffer(n2 * m2);

  auto gemm3_A = helper.GetCpuGpuBuffer(m3 * k3);
  auto gemm3_B = helper.GetCpuGpuBuffer(n3 * k3);
  auto gemm3_C = helper.GetCpuGpuBuffer(n3 * m3);

  auto gemm4_A = helper.GetCpuGpuBuffer(m4 * k4);
  auto gemm4_B = helper.GetCpuGpuBuffer(n4 * k4);
  auto gemm4_C = helper.GetCpuGpuBuffer(n4 * m4);




  auto gemm_group_A = helper.GetCpuGpuBuffer(m0 * k0 +
                                             m1 * k1 +
                                             m2 * k2 +
                                             m3 * k3 +
                                             m4 * k4
                                             );
  auto gemm_group_B = helper.GetCpuGpuBuffer(n0 * k0 +
                                             n1 * k1 +
                                             n2 * k2 +
                                             n3 * k3 +
                                             n4 * k4
                                             );
  auto gemm_group_C = helper.GetCpuGpuBuffer(n0 * m0 +
                                             n1 * m1 +
                                             n2 * m2 +
                                             n3 * m3 +
                                             n4 * m4
