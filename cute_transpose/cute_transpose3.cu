...
__global__ void device_kernel(int m, int n, void* a, void* b) {
  ...
  auto tensor_out = make_tensor(make_gmem_ptr((Element*)device_b), make_layout(in_shape, in_stride));
...
// tensor_in_tile.data();
  ...
  auto tensor_out_g = local_tile(tensor_out_tile, make_shape(_1{}, Int<Alignment>()), make_coord(ty, tx));
