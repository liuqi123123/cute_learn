...
template<typename Element, int Alignment, typename ThreadblockShape, typename G2SCopyA, typename S2GCopyA, typename SmemLayoutA>
__global__ void device_kernel(int m, int n, void* a, void* b) {
  ...
  auto tensor_smem = make_tensor(make_smem_ptr((Element*)seme), SmemLayoutA{});
...
void host_func(int m, int n, Element* host_a, Element* host_b, void* device_a, void* device_b){
  ...
  using AccessType = cutlass::AlignedArray<Element, align>;
  ...
  using SmemLayoutAtom = decltype(composition(Swizzle<5, 0, 5>{},
