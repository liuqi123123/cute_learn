...
__global__ void device_kernel(int m, int n, void* a, void* b) {
  ...
  // auto tensor_smem = make_tensor(make_smem_ptr(seme), SmemLayoutA{});
...
void host_func(int m, int n, Element* host_a, Element* host_b, void* device_a, void* device_b){
  ...
  using SmemLayoutAtom = decltype(composition(Swizzle<5, 0, 5>{},
                            make_layout(make_shape(Int<ThreadblockShape::kRow>{}, Int<ThreadblockShape::kColumn>{}),
                                        make_stride(Int<ThreadblockShape::kColumn>{}, Int<1>{}))));  // Atom Shape: (8, 64):(64, 1) swizzle(3, 3, 3)
  using SmemLayoutA = decltype(tile_to_shape(SmemLayoutAtom{}, make_shape(Int<ThreadblockShape::kRow>{}, Int<ThreadblockShape::kColumn>{})));   // (128, 64)
