...
template<typename Element, int Alignment, typename ThreadblockShape, typename G2SCopyA, typename S2GCopyA, typename SmemLayout>
__global__ void device_kernel(int m, int n, void* a, void* b) {
  ...
  auto out_stride = make_stride(m, _1{});
  ...
  // auto tensor_smem = make_tensor(make_smem_ptr((Element*)seme), make_layout(smem_shape, smem_stride));
  auto tensor_smem = make_tensor(make_smem_ptr((Element*)seme), SmemLayout{});
...
// tensor_in_tile.data();
  ...
  // auto tensor_out_g = local_tile(tensor_out_tile, make_shape( _1{}, Int<Alignment>()), make_coord(ty *2 + tx /4 ,tx % 4));

  //thread id to idx
  static const int out_thread_stride = (ThreadblockShape::kRow / Alignment);

  auto tensor_out_g = local_tile(tensor_out_tile, make_shape( _1{}, Int<Alignment>()), make_coord(idx / out_thread_stride ,idx % out_thread_stride));
...
void host_func(int m, int n, Element* host_a, Element* host_b, void* device_a, void* device_b){
  ...
  // using SmemLayoutAtom = decltype(composition(Swizzle<3, 2, 3>{},
  //                           make_layout(make_shape(Int<8>{}, Int<ThreadblockShape::kColumn>{}),
  //                                       make_stride(Int<ThreadblockShape::kColumn>{}, Int<1>{}))));

  using SmemLayoutAtom = decltype(composition(Swizzle<3, 2, 5>{},
                            make_layout(make_shape(Int<ThreadblockShape::kRow>{}, Int<ThreadblockShape::kColumn>{}),
                                        make_stride(Int<ThreadblockShape::kColumn>{}, Int<1>{}))));  //
  using SmemLayoutA = decltype(tile_to_shape(SmemLayoutAtom{}, make_shape(Int<ThreadblockShape::kRow>{}, Int<ThreadblockShape::kColumn>{})));


  device_kernel<Element, align, ThreadblockShape, G2SCopyA, S2GCopyA, SmemLayoutA><<<grid, block, 0, 0>>>(m, n, device_a, device_b);
}
...
int main() {
  ...
  auto b = composition(Swizzle<3, 1, 4>{}, a);
  print_layout(b);
